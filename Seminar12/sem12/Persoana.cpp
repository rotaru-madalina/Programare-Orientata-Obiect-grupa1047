...
Persoana::Persoana(int varsta, string nume, const char* telefon, double salariu)
{
	this->varsta = varsta;
...
void Persoana::scrieInFisierBinar(ofstream& iesire)
{
	...
	iesire.write((char*)&salariu, sizeof(salariu));
...
void Persoana::citireDinFisierBinar(ifstream& citire)
{
	...
	citire.read((char*)&salariu, sizeof(salariu));
...
double Persoana::calculeazaSalariuDupaMarireProcentuala(double procent)
{
	salariu += salariu * procent / 100 + 100;
	return salariu;
}

ostream& operator<<(ostream& iesire, Persoana& p)
{
	iesire << "Nume: " << p.nume << " varsta: " << p.varsta << " telefon: " << p.telefon << " salariu: " << p.salariu << endl;
...
{
	iesire << p.nume << endl << p.varsta << endl << p.telefon << endl << p.salariu << endl;
	return iesire;
}
